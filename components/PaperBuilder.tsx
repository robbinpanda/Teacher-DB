...
export function PaperBuilder({ questions }: { questions: Question[] }) {
  const params = useSearchParams();
  const requestedIds = params.get("ids")?.split(",").filter(Boolean) ?? [];
  const initial = requestedIds.length ? requestedIds : questions.slice(0, 5).map((question) => question.id);
  const [ids, setIds] = useState(initial);
  const [title, setTitle] = useState("九年级数学专题训练卷");
  const [subtitle, setSubtitle] = useState("满分：100 分　建议用时：90 分钟");
  const [showAnswers, setShowAnswers] = useState(false);
  const selected = useMemo(() => ids.map((id) => questions.find((question) => question.id === id)).filter(Boolean) as Question[], [ids, questions]);
  ...
  return (
    <div className="paper-builder">
      <header className="paper-topbar no-print">
        ...
        <div className="paper-save-state"><Check size={13} /> 已自动保存</div>
        <div className="header-actions"><button type="button" className="btn btn-small" onClick={() => setShowAnswers(!showAnswers)}><Eye size={14} /> {showAnswers ? "隐藏答案" : "答案预览"}</button><button type="button" className="btn btn-dark btn-small" onClick={() => window.print()}><Printer size={14} /> 打印 / 保存 PDF</button></div>
      </header>
          ...
          <article className="paper-sheet">
            ...
            {selected.map((question, index) => (
              <section className="paper-question" key={question.id}>
                ...
                {question.assets.length > 0 && <Image src="/demo-exam-page.svg" width={170} height={110} className="paper-crop-placeholder" alt={question.assets[0].label} />}
